...
public class AutoAssemblerMenu extends ModularUIContainerMenu {

    public AutoAssemblerMenu(MenuType<?> type, int containerId, Inventory playerInventory, IContainerUIHolder uiHolder) {
        super((MenuType)type, containerId, playerInventory, uiHolder);
    }

    public AutoAssemblerMenu(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf buf) {
        super((MenuType)com.example.factorycore.registry.CoreMenus.AUTO_ASSEMBLER_MENU.get(), containerId, playerInventory, getHolder(playerInventory, buf));
    }
